...
class TextVSImageMetric:
    ...
    def load_model(self, model_name: str, pretrained: str = "") -> None:
        ...
        if self._model is not None:
             logger.info("Text vs image alignment model already loaded.")
             return
        logger.info("Loading text vs image alignment model.")
        # Xác định precision dựa trên device
        self.model_precision = torch.float16 if self._device.type == 'cuda' else torch.float32
        logger.info(f"Using precision: {self.model_precision}")

        self._model, _, _ = open_clip.create_model_and_transforms(
            model_name, pretrained=pretrained, device=self._device, precision=str(self.model_precision).split('.')[-1] # 'float16' or 'float32'
        )
        self._tokenizer = open_clip.get_tokenizer(model_name)
        self._model.eval()
        logger.info(f"Model loaded to {self._device} with precision {self.model_precision}")
    ...
    def score_text_alignment_2d(
        ...
        clip_scores = torch.clip(clip_scores, 0, 1)
        clip_scores = clip_scores / 0.35
        return clip_scores.squeeze().tolist()
